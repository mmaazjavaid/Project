...
const AddSP = async (req, res) => {
    ...
    try {
        ...
        const spAlready = await SP.findOne({ email: req.body.email });
        if (spAlready == null) {

            const sp = new SP(
                { location, email, password, username, phone, skills, title, description, total_earnings, total_jobs, total_hours, education, experience, profile_image } = req.body
            )
